...
export function buildFigure(detailLevel: number): Figure {
  ...
  // silhouette reads as a head rather than as a polyhedron. Capped at 8
  // (~1.3M triangles), which is reached only at ultra: applyHair() walks every
  // vertex on the CPU at build time, so each level beyond that quadruples a
  // startup cost that is already the longest single build in the project.
  const headDetail = Math.min(8, Math.max(5, detailLevel));
